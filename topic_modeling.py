...
# ---- Shared LLM loader with 4-bit quantization fallback for low-VRAM GPUs ----
# Tries the following load strategies in order:
#   1. CUDA + 4-bit quantization (bitsandbytes) — uses ~2GB VRAM for 3B model
#   2. CUDA + FP16 — uses ~6GB VRAM
#   3. CPU — slow but works everywhere
LOW_VRAM_THRESHOLD_GB = 7.0  # GPUs below this get 4-bit by default

def _load_qwen_model(model_name: str, force_4bit: bool = False):
    """
    Load a HuggingFace causal LM with adaptive quantization for consumer hardware.
    Returns (model, tokenizer). Prints which strategy succeeded.
    """
    ...
    # Detect available VRAM to decide on quantization
    use_4bit = force_4bit
    if torch.cuda.is_available() and not use_4bit:
        try:
            ...
            if vram_gb < LOW_VRAM_THRESHOLD_GB:
                use_4bit = True
                print(f"  [Info] Detected {vram_gb:.1f}GB VRAM (< {LOW_VRAM_THRESHOLD_GB}GB) — enabling 4-bit quantization")
        except Exception:
            pass

    # Strategy 1: 4-bit quantization (bitsandbytes)
    if use_4bit and torch.cuda.is_available():
        try:
            ...
            print(f"  [Info] Loaded {model_name} in 4-bit (NF4) — ~2GB VRAM")
            return model, tokenizer
        except Exception as e:
            print(f"  [Info] 4-bit quantization unavailable ({e.__class__.__name__}: {e}); falling back to FP16")

    # Strategy 2: FP16 on CUDA, or CPU if no GPU
    device_map = "cuda" if torch.cuda.is_available() else "cpu"
    model_kwargs = dict(dtype=torch.float16, device_map=device_map)
    try:
        model_kwargs["attn_implementation"] = "flash_attention_2"
        model = AutoModelForCausalLM.from_pretrained(model_name, **model_kwargs)
        print(f"  [Info] Loaded {model_name} in FP16 with Flash Attention 2")
    except Exception:
        model_kwargs.pop("attn_implementation", None)
        model = AutoModelForCausalLM.from_pretrained(model_name, **model_kwargs)
        print(f"  [Info] Loaded {model_name} in FP16 (Flash Attention 2 unavailable)")
...
class HybridTopicModeler:
    ...
    def _try_bertopic(self, documents: List[str]) -> Dict:
        ...
        try:
            ...
            # Compute 2D per-document UMAP coords so the custom Document Cluster
            # view (in hand_heatmap_modern) can position each segment.
            doc_coords_2d = None
            try:
                from umap import UMAP
                n_n = min(15, max(2, len(filtered_docs) - 1))
                umap_2d = UMAP(n_neighbors=n_n, n_components=2, min_dist=0.1,
                               metric='cosine', random_state=42)
                doc_coords_2d = umap_2d.fit_transform(embeddings).tolist()
                print(f"  [OK] Computed 2D coords for {len(doc_coords_2d)} documents")
            except Exception as e:
                print(f"  [Warning] Could not compute 2D document coords: {e}")
            ...
            try:
                ...
                # DataMapPlot intentionally skipped — replaced by the redesigned
                # exemplar-based Document Cluster view in hand_heatmap_modern.py.
                viz_datamap = None
            ...
            bertopic_result = {
                ...
                'doc_topics': topics,  # per-document topic assignments
            }
  ...
  "The specific topic described is"
    ...
    def _generate_llm_labels_simple(self, topic_keywords_map: Dict[int, List[str]]) -> Dict[int, str]:
        ...
        try:
            ...
            print(f"  [Info] Loading {model_name} for NMF labeling...")
    ...
    def generate_topic_summaries(
        ...
        try:
            ...
            # 1. Load Model (adaptive quantization for consumer GPUs)
            model_name = "Qwen/Qwen2.5-3B-Instruct"
