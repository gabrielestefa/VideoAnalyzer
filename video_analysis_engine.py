...
class VideoAnalysisEngine:
    ...
    def process_video(self, sensitivity=0.1, progress_callback=None, enable_audio=False):
        ...
        frame_index = 0
        current_chunk = 0
        chunk_frames = []  # Temporary storage for current chunk
        ...
        while cap.isOpened():
            ...
            # Process the result (gesture detection, landmarks, etc.)
            frame_index += 1
            ...
            if progress_callback and frame_index % 30 == 0:
                ...
                progress_callback(overall_percent, f"Processing chunk {current_chunk + 1}/{num_chunks} \n ({len(chunk_frames)} frames)")
            
            # Check if chunk is complete
            if len(chunk_frames) >= frames_per_chunk or frame_index >= total_frames:
                # Only keep a sample of frames (every 10th frame) for display purposes
                # This prevents memory overflow while still allowing heatmap generation
                sample_rate = 10
                sampled_frames = chunk_frames[::sample_rate]
                self.frames.extend(sampled_frames)
        ...
        print("DEBUG: About to call analyze_episodes")
        self.analyze_episodes()
        print("DEBUG: analyze_episodes returned")
    ...
    def transcribe_audio(self, progress_callback=None):
        ...
        try:
            ...
            # Extract full audio to temp file
            import tempfile
            ...
            if FASTER_WHISPER_AVAILABLE:
                ...
                # model_size hardcoded to large-v3 for now as per previous logic
                p = multiprocessing.Process(target=_worker_whisper_transcribe, args=(temp_audio, "large-v3", queue))
                p.start()
                ...
                while True:
                    ...
                    try:
                        ...
                        if msg[0] == "progress":
                            ...
                            print(f"Transcription process error: {msg[1]}")
                            # Fallthrough to Google Speech
                            self.transcript = [] 
                    except:
                        pass
                ...
                if self.transcript:
                     print(f"  [faster-whisper] Transcribed {len(self.transcript)} sentences (Multiprocessing)")
                     # Cleanup temp audio
                     if os.path.exists(temp_audio): os.remove(temp_audio)
            ...
            for chunk_idx, t_start in enumerate(range(0, int(duration), chunk_len)):
                ...
                temp_chunk = f"temp_chunk_{t_start}.wav"
            ...
            import traceback
            traceback.print_exc()
        print("DEBUG: transcribe_audio finished")
    ...
    def get_context_at(self, seconds):
        ...
        g_l = self.gesture_history_left[idx][0] if idx < len(self.gesture_history_left) and self.gesture_history_left[idx] else "None"
        g_r = self.gesture_history_right[idx][0] if idx < len(self.gesture_history_right) and self.gesture_history_right[idx] else "None"
            ...
            for i in range(1, len(path)):
                ...
                if p_prev is not None and p_curr is not None:
                    ...
                    if 0 <= x < self.video_width and 0 <= y < self.video_height:
                         ...
                         self.speed_heatmap[max(0,y-5):min(self.video_height,y+5), 
                                            max(0,x-5):min(self.video_width,x+5)] += dist
    ...
    def get_text_embedding(self):
        ...
        try:
            ...
            model_path = r"D:\personal\OpenCV\VideoHandTracker\Models\universal_sentence_encoder.tflite"
    ...
    def analyze_episodes(self):
        ...
        print(f"DEBUG: analyze_episodes finished left hand. Found {len(self.episodes_left) if self.episodes_left else 0} episodes.")
